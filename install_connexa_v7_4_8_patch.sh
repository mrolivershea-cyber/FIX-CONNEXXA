...
echo "════════════════════════════════════════════════════════════════"
echo "  CONNEXA v7.4.8 - MULTI-TUNNEL AUTHENTICATION FIX"
echo "  Date: $(date '+%Y-%m-%d %H:%M:%S UTC')"
echo "  User: mrolivershea-cyber"
echo "  Updates: Base peers template, multi-tunnel MSCHAP-V2 support"
echo "════════════════════════════════════════════════════════════════"
...
# ============================================================================
echo ""
echo "📦 [Step 4/9] Installing pptp_tunnel_manager.py v7.4.8..."

cat > /app/backend/pptp_tunnel_manager.py <<'PYEOF'
"""
CONNEXA v7.4.8 - PPTP Tunnel Manager
...
class PPTPTunnelManager:
    """PPTP Tunnel Manager with v7.4.8 critical fixes."""
    
    def __init__(self):
        self.db_path = DB_PATH
        self.pppd_path = PPPD_PATH
        logger.info(f"PPTPTunnelManager v7.4.8 initialized")
    
    def create_tunnel(self, node_ip: str, username: str, password: str, 
                     node_id: int = None, socks_port: int = None) -> bool:
        """
        Create PPTP tunnel with all v7.4.8 critical fixes.
        ...
        logger.info(f"[v7.4.8] Creating tunnel for node {node_id} ({node_ip})")
...
echo "✅ pptp_tunnel_manager.py v7.4.8 installed"

# ============================================================================
# STEP 5: Install Watchdog Monitor (FIX #5)
# ============================================================================
echo ""
echo "📦 [Step 5/9] Installing watchdog.py v7.4.8..."
...
class WatchdogMonitor:
    ...
    def __init__(self, check_interval: int = 30):
        ...
        logger.info(f"WatchdogMonitor v7.4.8 initialized (check_interval={check_interval}s)")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='CONNEXA Watchdog Monitor v7.4.8')
    parser.add_argument(
        '--interval',
        type=int,
        default=30,
...
echo "✅ watchdog.py v7.4.8 installed"
...
echo ""
echo "✅ CONNEXA v7.4.8 Critical Fixes Installed"
echo ""
echo "Fixes Applied:"
...
echo "  - See docs/v7.4.8-fixes.md for detailed information"
echo "  - See SECURITY.md for security considerations"

echo ""
echo "════════════════════════════════════════════════════════════════"
echo "  ✅ CONNEXA v7.4.8 PATCH INSTALLATION COMPLETE"
echo "════════════════════════════════════════════════════════════════"

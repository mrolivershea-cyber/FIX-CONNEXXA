...
echo "════════════════════════════════════════════════════════════════"
echo "  CONNEXA SERVICE MANAGER v6.0 - PRODUCTION READY"
...
# ============================================================================
echo ""
echo "📦 Step 1/9: Installing packages..."
...
# ============================================================================
# STEP 2: Configure systemd service with resource limits
# ============================================================================
echo ""
echo "📦 Step 2/9: Configuring systemd service..."
...
# ============================================================================
# STEP 3: Create link_socks_to_ppp.sh
# ============================================================================
echo ""
echo "📦 Step 3/9: Creating link_socks_to_ppp.sh..."
...
echo "✅ link_socks_to_ppp.sh created and made executable"

# ============================================================================
# STEP 4: Create complete service_manager.py with verification
# ============================================================================
echo ""
echo "📦 Step 4/9: Creating service_manager.py..."
...
'''
    ...
    def _start_pptp_tunnel(self, node: Dict) -> Optional[str]:
        ...
        # Write configs
        peer_file = self._write_peer_conf(node_id, ip, username)
    ...
    def _get_diagnostics(self) -> Dict[str, Any]:
        ...
        syslog = self._run("grep -i 'pppd\\|Connect:' /var/log/syslog | tail -40 || true")[1]
    ...
    def start(self) -> Dict[str, Any]:
        """Start PPTP tunnels and SOCKS proxies with verification."""
    ...
    def stop(self) -> Dict[str, Any]:
        """Gracefully stop all PPTP tunnels and SOCKS proxies."""
        # Stop danted
        self._run("systemctl stop danted 2>/dev/null || true")
    ...
    def status(self) -> Dict[str, Any]:
        ...
        ppp_count = len(self._run("ip a | grep -c 'ppp[0-9]' || echo 0")[1].split())
        socks_ports = []
...
PYEOF
echo "✅ service_manager.py created with verification"

# ============================================================================
# STEP 5: Create router
# ============================================================================
echo ""
echo "📦 Step 5/9: Creating router..."
mkdir -p "$ROUTER_DIR"
...
# ============================================================================
echo ""
echo "📦 Step 6/9: Patching server.py..."
...
# ============================================================================
# STEP 7: Enable danted
# ============================================================================
echo ""
echo "📦 Step 7/9: Enabling danted..."
...
# ============================================================================
echo ""
echo "📦 Step 8/9: Configuring supervisor..."
if [ -f "/etc/supervisor/conf.d/connexa.conf" ]; then
    if ! grep -q "autorestart=true" "/etc/supervisor/conf.d/connexa.conf"; then
        sed -i '/\[program:backend\]/a autorestart=true\nstartretries=3' "/etc/supervisor/conf.d/connexa.conf"
    fi
    echo "✅ Supervisor configured with auto-restart"
fi
...
# ============================================================================
echo ""
echo "📦 Step 9/9: Restarting backend..."
...
if [ "$BACKEND_STATUS" = "RUNNING" ] || [ "$BACKEND_STATUS" = "active" ]; then
    ...
    echo "   supervisorctl tail backend stderr"
fi
...
echo "  1. Click 'Start Service' button"
echo "  2. Wait 20-30 seconds for tunnels to establish"
echo "  3. Check status with: curl http://localhost:8001/service/status"
echo ""
echo "Diagnostic commands:"
echo "  ip a | grep ppp                    # Show PPP interfaces"
echo "  netstat -tulnp | grep 108          # Show SOCKS ports"
echo "  tail -f /var/log/ppp/ppp-*.log     # Watch tunnel logs"
echo "  grep Connect: /var/log/syslog | tail -20  # Check connections"
